    ...
    def build_messages(self, messages: Sequence[ChatMessage]) -> list[dict[str, Any]]:
        ...
        for msg in messages:
            ...
            if msg.get("tool_call_id"):
                ...
                anthropic_msg["role"] = (
                    "user"  # Tool results are always user messages in Anthropic
                )
...
def create_anthropic_message_dict(
    wrapper: BaseChatResponse,
    ...
    if hasattr(response, "role") and hasattr(response, "content"):
        ...
        if response.content:
            ...
            if (
                ...
                and getattr(response.content[0], "type", None) == "text"
            ):
